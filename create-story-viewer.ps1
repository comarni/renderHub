...
$outputFolder = Join-Path (Get-Location) $projectName
$htmlFile = Join-Path $outputFolder "index.html"
...
for ($i = 1; $i -le $scenes; $i++) {
    $sceneData += [pscustomobject]@{
        ...
        position = [pscustomobject]@{x = ($i - 1) * 15; y = 0; z = -30}
        color = $colors[($i - 1) % $colors.Count]
    }
}

$sceneJson = ConvertTo-Json $sceneData -Depth 5
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        #info { position: absolute; top: 20px; left: 20px; color: #fff; z-index: 100; }
...
Write-Host "Pagina generada: $htmlFile" -ForegroundColor Green
Write-Host "Escenas: $scenes" -ForegroundColor Cyan
